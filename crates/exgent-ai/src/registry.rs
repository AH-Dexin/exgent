...
impl ProviderRegistry {
    pub fn builtin() -> Self {
        ...
        registry
            .register("openai-codex-responses", || {
                DynamicProvider::new("openai-codex-responses", OpenAiCodexResponsesProvider)
            })
            .expect("built-in provider adapters must be unique");
        registry
        ...
        registry
            .register("google-generative-ai", || {
                DynamicProvider::new("google-generative-ai", GoogleGenerativeAiProvider)
            })
            .expect("built-in provider adapters must be unique");
        registry
    ...
    pub fn with_dev_providers(mut self) -> Self {
        if !self.factories.contains_key("fake") {
            self.register("fake", || DynamicProvider::new("fake", FakeProvider))
...
mod tests {
    use super::*;
    use crate::{fake_model, ChatMessage, ProviderEvent, ProviderRequest};
